...
const App = () => {
  ...
  const { data: lowSecExecutedData, refetch: refetchLowScExecuted } = useQuery(GET_LOW_SEC_EXECUTED_TRANSACTIONS, {context: {chain: chainId},});
  const { data: lowSecCancelledData, refetch: refetchLowSecCancelled } = useQuery(GET_LOW_SEC_CANCELLED_TRANSACTIONS, {context: {chain: chainId},});
  const { data: lowSecSaltsData, refetch: refetchLowSecSalt } = useQuery(GET_LOW_SEC_SALTS, {context: {chain: chainId},});
  ...
  const { data: highSecExecutedData, refetch: refetchHighSecExecuted } = useQuery(GET_HIGH_SEC_EXECUTED_TRANSACTIONS, {context: {chain: chainId},});
  const { data: highSecCancelledData, refetch: refetchHighSecCancelled } = useQuery(GET_HIGH_SEC_CANCELLED_TRANSACTIONS, {context: {chain: chainId},});
  const { data: highSecSaltsData, refetch: refetchHighSecSalt } = useQuery(GET_HIGH_SEC_SALTS, {context: {chain: chainId},});
  ...
  }, [chainId, refetchLowSecScheduled, refetchLowScExecuted, refetchLowSecCancelled, refetchLowSecSalt, refetchHighSecScheduled, refetchHighSecExecuted, refetchHighSecCancelled, refetchHighSecSalt]);
  ...
  if (lowSecScheduledLoading || highSecScheduledLoading) return (
